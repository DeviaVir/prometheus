...
import (
	...
	"github.com/DeviaVir/prometheus/v2/util/stats"
)
...
func (ng *Engine) populateSeries(querier storage.Querier, s *parser.EvalStmt) {
	...
	parser.Inspect(s.Expr, func(node parser.Node, path []parser.Node) error {
		...
		case *parser.VectorSelector:
			...
			// sum of offset.
			// TODO(bwplotka): Add support for better hints when subquerying. See: https://github.com/DeviaVir/prometheus/v2/issues/7630.
			subqOffset, subqRange := ng.subqueryOffsetRange(path)
